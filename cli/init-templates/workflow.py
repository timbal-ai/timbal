...
workflow = (Workflow(name="demo_workflow")
    .step(fetch_data)
    .step(filter_data, csv_data=lambda: get_run_context().get_data("fetch_data.output"))
    .step(prettify_result, filtered_row=lambda: get_run_context().get_data("filter_data.output"))
)
