...
<ParamField path="embedding_names" type="string[]" required>
  List of embedding names to use for semantic search.
  <Warning>
    Querying with multiple embeddings is not supported yet, but the API is ready and this feature is coming soon.
  </Warning>
</ParamField>

<ParamField path="fts_columns" type="string[]">
  List of column names to use for full-text search. 
  <Note>
    When specified together with `embedding_names`, search will be hybrid (both semantic and full-text).
  </Note>
</ParamField>

<ParamField path="select_columns" type="string[]">
  List of column names to return in the search results. When not specified, all columns will be returned.
</ParamField>

<ParamField path="where_clauses" type="string[]">
  List of SQL WHERE clauses to apply to the search.
</ParamField>
