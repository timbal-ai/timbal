...
<AccordionGroup>
  <Accordion title="Installing Packages">
    Install Timbal with Slack integration support:
    ```bash
    pip install timbal[slack]
    ```
    ...
    <Note>
      The `[slack]` extra includes all necessary dependencies for complete Slack integration.
    </Note>
  </Accordion>  
...
First, configure your bot's user ID (found in your Slack app settings):

```python
SLACK_BOT_USER_ID = "U12345678"  # Replace with your actual bot user ID
```
...
```python
from timbal.handlers.slack.messages import send_message
from timbal.state import get_run_context
...
def post_hook():
    ...
    if reply.strip():
        send_message(
            ...
            text=reply,
            thread_ts=slack_thread_ts,  # Maintains thread context
        )
```
