---
title: "WhatsApp"
description: "Build a WhatsApp assistant with Timbal: webhook handling, persistence (KB/CSV), and media."
---

This guide shows how to create a WhatsApp assistant with Timbal that can:
- Respond to messages in real time via webhooks
- Persist conversations using Timbal Knowledge Bases or CSV
...
<AccordionGroup>
  <Accordion title="Meta WhatsApp App and Webhook">
    <Steps>
      <Step title="Create or open your WhatsApp App">
        Go to the Meta console and open your app's WhatsApp section: [Meta WhatsApp App Dashboard](https://developers.facebook.com/apps/796517512790442/whatsapp-business/wa-dev-console/?business_id=143503003811209).
      </Step>
  ...
  <Accordion title="Environment Variables">
    ...
    # Storage mode for this template: csv | timbal
...
def pre_hook():
    """Parse webhook, set prompt, and persist inbound to JSONL."""
    ...
    if not messages:
        return
    cm = _contact_name_map(contacts)
    msg = messages[0]
    from_number = msg.get("from")
    name = cm.get(from_number, from_number)
...
If you prefer using Timbal Knowledge Bases instead of CSV, you can import and query records with the platform helpers. These are async functions; adapt their usage to your environment accordingly.
...
<AccordionGroup>
  ...
  <Accordion title="No context/history">
    ...
    - If using CSV, verify the CSV file path and write permissions
  </Accordion>
</AccordionGroup>