...
Downloads request `Accept-Encoding: identity` and reject other HTTP content
encodings before reading the body, preventing decompression from bypassing the
host's file-size limit. Compressed files such as `.gz` remain supported when
served without HTTP content encoding; Python can decompress them in the sandbox.
