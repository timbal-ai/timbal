...
The agent still supplies only `code` and optional `dependencies`. Its description
...
Calls do not share variables or files. This tool does not mount the host filesystem,
export artifacts, or expose Timbal tools inside Python. It always uses Modal, with
no local execution fallback. Remote filesystem and persistent workspace support
