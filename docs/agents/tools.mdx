...
## Commands
Slash Commands are shortcuts to invoke tool calls directly.
When a user message begins with `/` followed by a command name, the Agent runs the tool directly without relying on the LLM to interpret and call the tool.

You can assign the command keyword to a tool via the `command` parameter.

```python highlight={9,17-18}
...
weather_tool = Tool(
    handler=get_weather,
    command="weather"  # This enables /weather command
)
...
await agent(prompt="/weather Paris").collect()
# "/weather Paris"  ->  get_weather(location="Paris")
```
