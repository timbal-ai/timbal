...
- Unknown or expired handles return a plain-text notice (not a tool error) telling the model the payload is gone and to re-run the tool instead of retrying the read.
...
class OffloadStore(Protocol):
    ...
    # optional: def exists(self, handle: str) -> bool  — enables stale-handle reconciliation
```
...
### Handles across turns and instances

Handles are only as durable as the store behind them. The default root is **per machine**: if a conversation's turns are served by different instances, or each turn runs in a fresh container, a later turn cannot read what an earlier one spilled to its own `~/.timbal/offload/`.

Two things keep that from costing iterations:

- **Shared root.** Set `TIMBAL_OFFLOAD_DIR` to a volume every instance mounts (a network filesystem, a persistent disk). `LocalOffloadStore()` resolves its root as: explicit `root=` argument → `$TIMBAL_OFFLOAD_DIR` → `~/.timbal/offload`.
- **Reconciliation at turn start.** When memory is loaded from the previous run, every inherited placeholder is probed against the current store (`exists(handle)`, one `stat`). Handles the store cannot resolve are rewritten to an explicit "no longer readable — re-run the tool" note and their `offload_handle` is cleared, so the model is never invited to page a dead payload. The cleared handles are recorded under `metadata["offload_unreadable"]` on the agent span. Custom stores opt in by implementing `exists(handle) -> bool` (sync or async); stores without it are left untouched.
