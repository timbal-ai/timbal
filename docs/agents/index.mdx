...
| `on_max_iter` | Extra LLM call | Output | Use when |
|---|---|---|---|
| `"final_answer"` (default) | One, with **no tools** | The model's text | Chat: the user should get an answer, even a partial one |
| `"stop"` | None | `max_iter_stop_message` as the assistant reply (`stop_reason="max_iter"`) | You want to decide what to show the user yourself, at zero extra cost |
| `"error"` | None | Raises `timbal.errors.MaxIterExceeded` | Batch / pipelines: a partial answer is worse than a failure |

In `"final_answer"` mode the agent first appends `max_iter_notice` as a user message telling the model its tools are gone and asking it to report what was and was not completed — otherwise models tend to narrate the step they were about to take ("I'll update the file now") as if it had happened.
...
agent = Agent(
    ...
    max_iter=16,
    on_max_iter="final_answer",
    # ``{max_iter}`` is substituted in both messages. ``max_iter_notice=None`` skips the notice.
    max_iter_notice="[System] Step budget of {max_iter} spent. Summarize progress; do not claim unfinished work is done.",
    max_iter_stop_message="[Stopped after {max_iter} steps without a final answer.]",  # used by on_max_iter="stop"
)
```

Every mode records `metadata["max_iter_reached"] = {"max_iter", "iterations", "behavior"}` on the agent span, so exhausted turns are easy to find in traces. The notice and the stop message are persisted in memory like any other message, tagged with `metadata={"source": "runtime", "kind": "max_iter_notice" | "max_iter_stop"}` so `Message.is_runtime()` lets you hide them when rendering a human transcript. `"error"` is meant for one-shot runs: memory is left ending in an unanswered tool batch, so a chained next turn starts from that dangling state.
