...
Use `FallbackModel` when you want an agent to try another model if the primary provider fails, including authentication errors, rate limits, timeouts, and server errors.
...
Models are tried in order. By default, each model gets up to two retries for transient errors. Authentication errors trigger fallback immediately, as do rate limits when another model is available. If every model fails, Timbal raises `FallbackExhausted` with the per-model errors.
...
The agent's `api_key` and `base_url` apply only to models from the primary provider. When the chain switches providers, Timbal resolves that provider's own credentials and endpoint from its environment variables or the platform proxy. Use `ModelEntry(api_key=..., base_url=...)` to override these for an individual model, including when multiple providers share a gateway.
...
`max_iter` (default `10`) bounds the LLM → tool rounds in one turn. `on_max_iter` decides what happens when the budget is spent and the model still wants tools:

| `on_max_iter` | Extra LLM call | Output |
|---|---|---|
| `"final_answer"` (default) | One, with **no tools**, preceded by a notice telling the model its budget is gone | The model's text |
| `"stop"` | None | A fixed assistant reply (`stop_reason="max_iter"`) |
| `"error"` | None | Raises `timbal.errors.MaxIterExceeded` — for one-shot runs; memory is left ending in an unanswered tool batch |

The notice matters: silently handed an empty tool list, models narrate the step they were about to take ("I'll update the file now") as if it had happened.

For anything custom, pass a callable (sync or async). It receives a `MaxIterContext(max_iter, memory, agent_path)` and returns a message: `role="user"` is used as the notice for a final tool-less call, `role="assistant"` is the reply and ends the turn, `None` makes the tool-less call with no notice, and raising fails the run.

```python
def out_of_budget(ctx):
    if ctx.agent_path != "assistant":  # sub-agents: stop cheaply, let the parent decide
        return {"role": "assistant", "content": f"[Stopped after {ctx.max_iter} steps.]"}
    return {"role": "user", "content": "Budget spent. Report what is done and what is not; claim nothing else."}

agent = Agent(name="assistant", model="openai/gpt-5", tools=[search, write_report], max_iter=16, on_max_iter=out_of_budget)
```

Every mode records `metadata["max_iter_reached"] = {"max_iter", "behavior"}` on the agent span. Injected messages are persisted in memory tagged `metadata={"source": "runtime", "kind": "max_iter_notice" | "max_iter_stop"}`, so `Message.is_runtime()` lets you hide them when rendering a human transcript.

### Timeout

`max_iter` bounds steps; `timeout` bounds time. It is the wall-clock budget, in seconds, for one turn — every LLM call and tool call included. Default `None` (no limit).

```python
agent = Agent(name="assistant", model="openai/gpt-5", tools=[search, write_report], timeout=120)

result = await agent(prompt="Research and write the report").collect()
if result.status.code == "timeout":
    print(result.error["message"])  # "assistant timed out after 120s"
    partial = result.output         # whatever the turn produced before the deadline
```

When the deadline passes, the in-flight LLM or tool call is cancelled and the turn ends with `status.code == "timeout"` and an `error` of type `RunTimeout`. Memory stays consistent: an interrupted tool call gets a synthetic result, so the next turn runs normally.

`timeout` is available on every runnable, not just agents:

- **Sub-agents and tools.** A child's own `timeout` fails only that call. The parent agent receives it as an error tool result and can recover. A `timeout` on the parent bounds the whole turn, children included.
- **Workflow steps.** A step that times out blocks its dependents, including transitive dependents. Unrelated branches may finish before the workflow reports the step failure. An agent inside a workflow can still recover from its own tools' timeouts; the workflow uses that agent's final result. See [Errors & failure](/workflows/errors-and-failure#timeouts).
- **Hooks.** `pre_hook`, the handler, and `post_hook` share one deadline. A slow hook times out the call; the budget is not restarted between stages. If the handler finished before a post-hook timed out, its output is retained.
- **What doesn't count.** The clock starts after input validation and the approval gate, so time a run spends paused waiting for approval does not use up the budget. Children detached with `background_mode` use `background_timeout` instead (see [Background Tasks](/agents/background-tasks#timeouts)).
- **Sync handlers.** A sync tool running inline on the event loop can't be interrupted. Make it async, or set `offload_blocking=True`.

A `TimeoutError` raised inside a handler, such as a slow HTTP call, is still reported as a regular `error`. Only the runnable's own deadline produces `timeout`.

**Performance.** An unset timeout has negligible overhead: the per-call check remains, but the stream relay and timers are bypassed. Enabling a timeout adds timer and relay work per streamed event. The `benchmarks/timeout/bench_streaming.py` benchmark measures this cost separately from network and LLM latency.
