...
Every mode records `metadata["max_iter_reached"] = {"max_iter", "iterations", "behavior"}` on the agent span, so exhausted turns are easy to find in traces. The notice and the stop message are persisted in memory like any other message.
