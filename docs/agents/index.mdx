...
7. **[Human in the loop](/agents/human-in-the-loop)** — approvals, `suspend()`, durable resume — usually needs a UI and tracing wired up
...
Some models support extended thinking before responding. Check which models support it on [Model Capabilities](/model_capabilities).
...
The full reference — durable cross-process resume, audit fields, redaction, parallel gates, `pending_approvals()`, status reasons, usage counters, plus `suspend()` for asking the user mid-run — lives on the dedicated [Human in the Loop](/agents/human-in-the-loop) page.
