...
<Tip>
Order matters: rails run in list order (block-only rails are checked concurrently for latency, but the first non-allow verdict in list order wins, and mutating rails each see the previous rail's transformation). Put normalizing/redacting rails before judging rails.
</Tip>
