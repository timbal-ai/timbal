...
<CardGroup cols={2}>
  ...
  <Card title="gpt-6-sol">
  `openai/gpt-6-sol`

  For complex coding and agentic workflows.

  - &#36;2 / &#36;10 per 1M tokens (input / output); cached input &#36;0.20; cache writes &#36;2.50
  - Over 272K input tokens: &#36;4 / &#36;15; cached input &#36;0.40; cache writes &#36;5, applied to the full request
  - <Icon icon="window-maximize" size={14} /> 1.05M context
  - <Icon icon="right-from-bracket" size={14} /> 128K max output
  - <Icon icon="keyboard" size={14} /> Text and Image input; Text output
  - Streaming, function calling, and structured outputs
  - <Icon icon="brain" size={14} /> Reasoning: `none`, `low`, `medium` (default), `high`, `xhigh`, `max`
  - <Icon icon="calendar" size={14} /> Knowledge cutoff Apr 20, 2026
  </Card>

  <Card title="gpt-6-luna">
  `openai/gpt-6-luna`

  For focused tasks and high-volume workloads at low cost.

  - &#36;0.10 / &#36;0.50 per 1M tokens (input / output); cached input &#36;0.01; cache writes &#36;0.125
  - Over 272K input tokens: &#36;0.20 / &#36;0.75; cached input &#36;0.02; cache writes &#36;0.25, applied to the full request
  - <Icon icon="window-maximize" size={14} /> 1.05M context
  - <Icon icon="right-from-bracket" size={14} /> 128K max output
  - <Icon icon="keyboard" size={14} /> Text and Image input; Text output
  - Streaming, function calling, and structured outputs
  - <Icon icon="brain" size={14} /> Reasoning: `none`, `low`, `medium` (default), `high`, `xhigh`, `max`
  - <Icon icon="calendar" size={14} /> Knowledge cutoff May 18, 2026
  </Card>
</CardGroup>

### Using GPT-6 Sol and Luna

Set `OPENAI_API_KEY` and choose `openai/gpt-6-sol` or `openai/gpt-6-luna`:
...
agent = Agent(
    name="assistant",
    model="openai/gpt-6-sol",  # Or openai/gpt-6-luna
    max_tokens=4096,
    model_params={"reasoning": {"effort": "medium"}},
)
```

Use the Responses API (Timbal's default) for reasoning with tools. With `TIMBAL_OPENAI_API=chat_completions`, function calling on Sol and Luna requires `model_params={"reasoning_effort": "none"}`. Astra still requires Responses for tool calling.

All GPT-6 prices shown are Standard rates. Batch and Flex cost half the applicable rates; Fast mode costs twice as much, including cache and long-context rates. Opt into Flex or Fast with `model_params={"service_tier": "flex"}` or `{"service_tier": "fast"}`; metering follows the tier reported by the provider. Regional processing adds 10% where eligible. Sol and Luna support EU data residency only with Standard processing.

Sol and Luna launched September 22, 2026. Specs and prices verified September 23, 2026 against [Sol's model reference](https://developers.openai.com/api/docs/models/gpt-6-sol), [Luna's model reference](https://developers.openai.com/api/docs/models/gpt-6-luna), and [OpenAI pricing](https://developers.openai.com/api/docs/pricing).

### How long-context and cache pricing is tracked

GPT-6 Astra/Sol/Luna, GPT-5.6 Sol/Terra/Luna, GPT-5.5, GPT-5.4 and GPT-5.4 Pro bill the **entire request** at long-context rates once the prompt exceeds **272K input tokens** — including cache reads, cache writes, and output. (GPT-5.5 Pro's model card publishes no long-context tier and no cached-input discount, so it is billed flat.) Timbal's collectors decide the tier from the raw prompt size reported by OpenAI (cache hits included; exactly 272K is still short context) and emit distinct usage units so each tier can be priced separately:
