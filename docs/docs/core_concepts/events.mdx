...
> The `path`, `call_id`, and `run_id` fields help track execution flow and will be explained in detail in the [Tracing](./tracing.mdx) section.
...
:::note[Note on persistence]
Events are **not persisted to memory** - they are temporary notifications that stream during execution and are consumed in real-time. For permanent storage and analysis, this information is captured in **Traces**, which will be explained in detail in the following sections.
:::
