...
<CodeBlock language="python" highlight="7" code={`from datetime import datetime
...
:::info[Accessing other traces]{.tight-admonition}
...
<CodeBlock language="python" highlight="19" code={`from datetime import datetime
...
<CodeBlock language="python" highlight="13" code={`def post_hook():
    trace = get_run_context().current_trace()
