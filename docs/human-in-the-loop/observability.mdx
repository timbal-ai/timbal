...
- `approvals:remembered` — an approved resolution carried `scope="session"` and a grant was stored
- `approvals:granted` — a gate was approved by an existing session grant, without emitting an `ApprovalEvent`
