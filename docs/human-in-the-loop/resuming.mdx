...
<Note>
Compaction runs at **turn boundaries** (when a turn loads its memory), not between iterations within a turn. A single turn that itself balloons — many large tool results in one uninterrupted run — is not compacted mid-loop, on resume or otherwise. This is a general limitation, not specific to human-in-the-loop; size your individual tool outputs accordingly or compact across turns.
</Note>
