...
See the full reference on the [Human in the Loop](/agents/human-in-the-loop) page. This page is a catalog of shapes you can copy.
...
def ask_multi_choice(question: str, options: list[str]) -> list[str]:
    ...
    return suspend({"question": question, "options": options}, kind="ask_multi_choice")
...
`suspend` is exported at the top level (`from timbal import suspend`). Ready-made `ask_user` and `confirm` also ship in `timbal.tools` (`from timbal.tools import ask_user, confirm`).

<Warning>
The handler **re-executes from the top** on resume, so put `suspend()` before any non-idempotent side-effect (or make everything before it idempotent). For irreversible actions, gate them with [`requires_approval`](/agents/human-in-the-loop#approval-gates) instead — that pauses *before* any handler code runs.
</Warning>
...
| `ask_multi_choice` | `{ "question": str, "options": [str] }` | `[str]` |
...
agent = Agent(
    ...
    model="openai/gpt-5",
    tools=[ask_text, ask_choice, ask_multi_choice, confirm, ask_form, ask_rating],
    system_prompt=(
...
See [Human in the Loop → Resuming a paused run](/agents/human-in-the-loop#resuming-a-paused-run) for the durable providers and the HTTP `/stream` wire contract your frontend talks to.
