...
class VoiceAgent(Agent):
    ...
    def __init__(
        ...
        audio_format: str = "pcm16",    # pcm16, g711_ulaw, or g711_alaw
    ...
    async def audio_chunk_generator(self, wave_file: str, samplewidth: int, chunk_size: int):
    ...
    async def combine_samples(self, sample1: bytes, sample2: bytes, intensity: float=0.3):
        def decode_sample(sample: bytes):
            if sample:
                return np.frombuffer(sample, dtype=np.int16).astype(np.int32)
            return None

        def encode_sample(sample: np.ndarray):
            return np.clip(sample, -32768, 32767).astype(np.int16).tobytes()

        s1 = decode_sample(sample1)
        s2 = decode_sample(sample2)

        if s1 is None:
            return encode_sample(s2 * intensity)
        
        mixed = s1 + s2 * intensity
        return encode_sample(mixed)
    ...
    async def session(self, input_stream: AsyncGenerator[Any, None]) -> AsyncGenerator[Any, None]:
        ...
        async def _consume_elevenlabs_audio_queue():
            bg_audio_gen = self.audio_chunk_generator("../office-ambience-16khz.wav", samplewidth=2, chunk_size=640)
            chunk_buffer = bytearray()
            chunk_size_bytes = 640  # 20ms at 16kHz (16000 samples/sec * 0.02 sec = 320 samples * 2 bytes = 640 bytes)
            sleep_time = 0.02  # 20ms for real-time playback (matches chunk duration)
            bg_intensity = 0.5
