...
async def semantic_steps_validator(
    ...
    step_response = await agent_eval_steps.complete(prompt=prompt)
        ...
        if isinstance(v, dict):
            ...
            for validator, ref in v.items():
                if validator == "contains":
                    ...
                    validators.append(lambda output, ref=ref: (all([x in output for x in ref]), f"Must contain: {ref}", output, ref))
                    ...
                    validators.append(lambda output, ref=ref: (bool(re.search(ref, output, re.MULTILINE)), f"Must match regex: {ref}", output, ref))
                elif validator == "exactly":
                    # TODO: specific type?
                    validators.append(lambda output, ref=ref: (output == ref, f"Must be exactly: {ref}", output, ref))
                elif validator == "semantic":
                    if isinstance(ref, str):
                        async def validator_fn(output):
                            return await semantic_output_validator(output, ref)
                        validators.append(validator_fn)
                    elif isinstance(ref, list):
                        async def validator(output):
                            for expected_output in ref:
                                correct, explanation, actual, expected = await semantic_output_validator(output, expected_output)
                                if correct:
                                    return True, explanation, actual, expected
                            # If none matched, return the last explanation
                            return False, f"No semantic outputs matched: {ref}", output, ref
                        validators.append(validator)
                    else:
                        raise ValueError("semantic must be a string or list of strings")
                else:
...
class Turn(BaseModel):
        ...
        if isinstance(v, str):
            ...
            for validator, ref in v.items():
                ...
                if validator == "contains":
                    ...
                    if not isinstance(ref, list):
                        raise ValueError("contains must be a list")
                    validators.append(lambda steps, ref=ref: (all([x in steps for x in ref]), f"Must contain: {ref}", ref))
                elif validator == "semantic":
    ...
    def validate_output(cls, v):
        if isinstance(v, dict):
            return Output.model_validate(v)
        elif isinstance(v, str):
            # Aquí default semantic?
            return Output(validators={"semantic": v})
        raise ValueError("output must be a string or dict")
...
async def eval_output(
    ...
    for validator in turn.output.validators:
        if inspect.iscoroutinefunction(validator):
            ...
            correct, explanation, actual, expected = validator(agent_output)
        results.append(correct)
        explanations.append(explanation)
