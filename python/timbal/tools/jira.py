"""Jira Cloud REST API tools (Atlassian OAuth 2.0 / 3LO).
...
async def _jira_request(
    ...
    *,
    ...
    token = await _resolve_token(tool)
    url = f"{_jira_api_root(cid)}{path}"
    hdrs: dict[str, str] = {"Authorization": f"Bearer {token}", "Accept": "application/json"}
    if headers:
        hdrs.update(headers)
    async with httpx.AsyncClient() as client:
...
class JiraListIssues(Tool):
    ...
    def __init__(self, **kwargs: Any) -> None:
        async def _handler(
            ...
            if expand:
                params["expand"] = expand
            return await _jira_request(self, "GET", cloud_id, site_name, "/rest/api/3/search", params=params)
...
class JiraExecuteJql(Tool):
    ...
    description: str | None = "Run a raw JQL query against /rest/api/3/search (advanced search)."
    integration: Annotated[str, Integration("jira")] | None = None
    token: SecretStr | None = None
...
class JiraAddAttachment(Tool):
    ...
    def __init__(self, **kwargs: Any) -> None:
        async def _handler(
            ...
            cid = await _resolve_cloud_id(self, cloud_id, site_name)
            token = await _resolve_token(self)
            url = f"{_jira_api_root(cid)}/rest/api/3/issue/{issue_id_or_key}/attachments"
            headers = {
    ...
    def __init__(self, **kwargs: Any) -> None:
        async def _handler(
            ...
            if role == "watcher":
                cid = await _resolve_cloud_id(self, cloud_id, site_name)
                token = await _resolve_token(self)
                url = f"{_jira_api_root(cid)}/rest/api/3/issue/{issue_id_or_key}/watchers"
                headers = {
                    "Authorization": f"Bearer {token}",
                    "Accept": "application/json",
...
class JiraGetMyRecentActivity(Tool):
    ...
    def __init__(self, **kwargs: Any) -> None:
        async def _handler(
            ...
            params = {"jql": jql, "startAt": 0, "maxResults": max_results}
            return await _jira_request(self, "GET", cloud_id, site_name, "/rest/api/3/search", params=params)
