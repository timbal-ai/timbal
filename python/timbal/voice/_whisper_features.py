...
Vendored from Pipecat's ``pipecat.audio.turn.smart_turn._whisper_features``
(BSD 2-Clause, Copyright (c) 2024-2026, Daily), itself a numpy port of
``transformers.WhisperFeatureExtractor`` (Apache-2.0). Kept dependency-free
beyond numpy so ``timbal[voice]`` stays onnxruntime + huggingface_hub only.

Requires the ``timbal[voice]`` extra (numpy ships with onnxruntime).
"""
...
def _power_spectrogram(
    ...
    # decimate by `hop_length` to land on the analysis frames.
...
def compute_whisper_log_mel_features(
    ...
    *,
    ...
    The caller should already have padded or truncated the input to exactly
    ...
    Args:
        audio: One-dimensional float audio at 16 kHz. Any numeric dtype is
            accepted; the function casts internally and returns float32.
    ...
    # Done in float32 to match the reference's precision.
    if do_normalize:
