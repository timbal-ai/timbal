...
class TestBackgroundTasks:
    ...
    async def test_background_task_execution_and_status(self):
        ...
        result = await agent(prompt="Run 'sleep 5'").collect()
        ...
        status_result = await agent(prompt="What is the status of your background tasks?").collect()
        ...
        final_status = await agent(prompt="Check the status of your background tasks again").collect()
    ...
    async def test_bg_tasks_dict_populated_when_running_background(self):
        ...
        result = await agent(prompt="Run 'sleep 1 && echo test' in the background").collect()
    ...
    async def test_agent_has_get_background_task_tool(self):
        ...
        async for event in agent(prompt="Run 'sleep 0.5' in the background"):
            if (
                ...
                and event.path == "tool_check_agent.llm"
    ...
    async def test_background_task_with_events_and_logs(self):
        ...
        result = await agent(prompt="Build a project called 'my-app' in the background").collect()
        ...
        status_result = await agent(prompt="What is the status of the build task?").collect()
    ...
    async def test_realtime_events_vs_background_events(self):
        ...
        if len(bg_agent_runnable._bg_tasks) > 0:
            ...
            assert queued_event_count > 0, "Background task events should be queued, not streamed to caller"
    ...
    async def test_multiple_background_tasks_concurrently(self):
        ...
        result1 = await agent(prompt="Run 'sleep 1 && echo task1' in the background").collect()

        assert_has_output_event(result1)

        # Start second background task while first is running
        result2 = await agent(prompt="Run 'sleep 1 && echo task2' in the background").collect()
    ...
    async def test_background_task_error_handling(self):
        ...
        fail_tool = Tool(
            name="failing_tool", description="A tool that can fail", handler=failing_tool, background_mode="auto"
        )
        ...
        result = await agent(prompt="Run failing_tool with should_fail=True in the background").collect()
        ...
        status_result = await agent(prompt="Check the status of the background task").collect()
    ...
    async def test_background_mode_always(self):
        ...
        result = await agent(prompt="Use always_bg_tool with message='hello'").collect()
    ...
    async def test_background_mode_never(self):
        ...
        # Execute
        result = await agent(prompt="Use never_bg_tool with message='test'").collect()
    ...
    async def test_background_task_cleanup_after_completion(self):
        ...
        quick_tool = Tool(name="quick_task", description="Quick task", handler=quick_task, background_mode="auto")
        ...
        # Start background task
        result = await agent(prompt="Run quick_task with value='test' in the background").collect()
        ...
        status_result = await agent(prompt="Check the status of background tasks").collect()
    ...
    async def test_background_task_with_structured_output(self):
        ...
        # Start background task
        result = await agent(prompt="Run structured_task with count=3 in the background").collect()
        ...
        # Check final status
        final_result = await agent(prompt="Get the final status of the background task").collect()
    ...
    async def test_background_task_long_running_check_multiple_times(self):
        ...
        result = await agent(prompt="Run 'sleep 2 && echo completed' in the background").collect()
        ...
        for i in range(3):
            await asyncio.sleep(0.3)
            status_result = await agent(prompt=f"Check status of background tasks (check {i + 1})").collect()
    ...
    async def test_background_task_events_accumulation(self):
        ...
        event_tool = Tool(
            name="event_gen_tool", description="Generates events", handler=event_generating_task, background_mode="auto"
        )
        ...
        result = await agent(prompt="Run event_gen_tool with num_events=5 in the background").collect()
        ...
        status_result = await agent(prompt="Check status and get events from the background task").collect()
    ...
    async def test_background_task_in_conversation_context(self):
        ...
        intro_result = await agent(prompt="I'm going to ask you to run a task in the background next").collect()

        assert_has_output_event(intro_result)

        # Second turn: start background task
        bg_result = await agent(prompt="Now run 'sleep 1 && echo done' in the background").collect()
        ...
        remember_result = await agent(prompt="What task did I just ask you to run?").collect()
    ...
    async def test_get_background_task_tool_parameters(self):
        ...
        result = await agent(prompt="Run 'sleep 0.5' in the background").collect()
    ...
    async def test_background_task_with_bash_command_output(self):
        ...
        result = await agent(prompt="Run 'echo \"Hello from background\"' in the background").collect()
        ...
        # Check final status
        status_result = await agent(prompt="What was the output of the background command?").collect()
    ...
    async def test_background_task_event_queue_retrieval(self):
        ...
        multi_tool = Tool(
            name="multi_step", description="Multi-step task", handler=multi_step_task, background_mode="auto"
        )
        ...
        # Start background task
        result = await agent(prompt="Run multi_step with steps=3 in the background").collect()
        ...
        if len(agent_runnable._bg_tasks) > 0:
            ...
            assert isinstance(task_info["event_queue"], asyncio.Queue), "Event queue should be an asyncio.Queue"
    ...
    async def test_background_tasks_persist_across_agent_calls(self):
        ...
        result1 = await agent(prompt="Run 'sleep 1' in the background").collect()

        assert_has_output_event(result1)

        # Immediately make another call to check status
        result2 = await agent(prompt="What background tasks are running?").collect()
