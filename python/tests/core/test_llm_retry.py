...
class TestRetryOnError:
    ...
    async def test_server_error_503_retries(self):
        ...
        async def server_error_stream():
            ...
            if attempt_count < 2:
                ...
                raise AnthropicAPIStatusError(message="Service unavailable", response=response, body=None)
    ...
    async def test_server_error_500_retries(self):
        ...
        async def server_error_stream():
            ...
            if attempt_count < 2:
                ...
                raise OpenAIAPIStatusError(message="Internal server error", response=response, body=None)
    ...
    async def test_client_error_400_no_retry(self):
        ...
        async def client_error_stream():
            ...
            raise OpenAIAPIStatusError(message="Bad request", response=response, body=None)
    ...
    async def test_auth_error_401_no_retry(self):
        ...
        async def auth_error_stream():
            ...
            raise AnthropicAPIStatusError(message="Unauthorized", response=response, body=None)
            yield  # Make it a generator
    ...
    async def test_exponential_backoff(self):
        """Test that capped exponential backoff is passed to jitter."""
        delays = []
        ...
        assert mock_jitter.call_args_list[0].args == (0, 1.0)
        assert mock_jitter.call_args_list[1].args == (0, 2.0)
        assert mock_jitter.call_args_list[2].args == (0, 4.0)
        assert delays == [1.0, 2.0, 4.0]

    @pytest.mark.asyncio
    async def test_retry_after_header_is_delay_floor(self):
        """Test that Retry-After overrides shorter jitter delays."""
        response = MagicMock()
    ...
    async def test_invalid_retry_after_uses_jitter_delay(self):
        ...
        response = MagicMock()
        response.headers = {"Retry-After": "nope"}
        delays = []
