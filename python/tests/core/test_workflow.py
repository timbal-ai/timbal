...
class TestStepManagement:
    ...
    async def test_parameters_same_name_different_steps(self):
        ...
        result = await wf(x="workflow_param").collect()
        assert get_run_context().tracing.as_records()[0].input['x'] == "workflow_param"
        assert get_run_context().tracing.as_records()[1].input['x'] == "workflow_param"
        assert get_run_context().tracing.as_records()[2].input['x'] == "workflow_param"
        assert result.output == "async:workflow_param"
...
class TestContext:
    ...
    async def test_context_levels(self):
        ...
        _ = await wf().collect()
        assert get_run_context().tracing.as_records()[0].input == {}
        assert get_run_context().tracing.as_records()[1].input['x'] == "step_param"
        ...
        _ = await wf().collect()
        assert get_run_context().tracing.as_records()[1].new_parameter == "new_parameter"   # 0: workflow, 1: step
        assert get_run_context().tracing.as_records()[1].second_new_parameter == "second_new_parameter"
    ...
    async def test_multiple_outputs(self):
        ...
        _ = await wf().collect()
        assert len(get_run_context().tracing.as_records()[0].output) == 2
        assert get_run_context().tracing.as_records()[0].output[0] == "first_output"
        assert get_run_context().tracing.as_records()[0].output[1] == "second_output"
...
class TestControlFlow:
    ...
    async def test_parallel_execution(self):
        ...
        _ = await wf().collect()
        assert get_run_context().tracing.as_records()[0].output == "async:output"
    ...
    async def test_depends_on(self):
        ...
        _ = await wf().collect()
        assert get_run_context().tracing.as_records()[0].output == "sync:output"
    ...
    async def test_when_false(self):
        ...
        _ = await wf().collect()
        # print(get_run_context().tracing.as_records())
        assert len(get_run_context().tracing.as_records()) == 2 # sync step never called
        assert get_run_context().tracing.as_records()[0].output == "async:output"
    ...
    async def test_automatic_dependency_linking(self):
        ...
        _ = await wf().collect()
        print(get_run_context().tracing.as_records())
        assert get_run_context().tracing.as_records()[0].output == "new_parameter_value"
