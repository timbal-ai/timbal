...
class TestDefaultParamsBasic:
    ...
    def test_tool_with_static_default_params(self):
        ...
        tool = Tool(
            name="static_tool",
            handler=handler,
            default_params={"prefix": "static_value"}
        )
    ...
    def test_tool_with_callable_default_params(self):
        ...
        tool = Tool(
            name="callable_tool",
            handler=handler,
            default_params={"time_str": get_current_time}
        )
...
class TestDefaultParamsValidation:
    ...
    def test_valid_async_parameterless_function(self):
        ...
        tool = Tool(
            name="async_tool",
            handler=handler,
            default_params={"timestamp": async_get_timestamp}
        )
    ...
    def test_function_with_optional_params(self):
        ...
        tool = Tool(
            name="optional_tool",
            handler=handler,
            default_params={"context": get_context_info}
        )
    ...
    def test_invalid_function_with_required_params(self):
        ...
        with pytest.raises(ValueError, match="Callable must not have any required parameters"):
            Tool(
                name="invalid_tool",
                handler=handler,
                default_params={"value": get_required_param}
            )
    ...
    def test_invalid_non_callable(self):
        ...
        tool = Tool(
            name="non_callable_tool",
            handler=handler,
            default_params={"value": "not_a_function"}
        )
...
class TestDefaultParamsExecution:
    ...
    async def test_static_default_params_execution(self):
        ...
        tool = Tool(
            name="static_exec_tool",
            handler=handler,
            default_params={"prefix": "DEFAULT"}
        )
    ...
    async def test_callable_default_params_execution(self):
        ...
        tool = Tool(
            name="callable_exec_tool",
            handler=handler,
            default_params={"counter": get_counter}
        )
...
class TestDefaultParamsEdgeCases:
    ...
    async def test_callable_returning_none(self):
        ...
        tool = Tool(
            name="none_tool", 
            handler=handler,
            default_params={"value": return_none}
        )
    ...
    async def test_mixed_none_and_valid_results(self):
        ...
        tool = Tool(
            name="mixed_none_tool",
            handler=handler,
            default_params={
                "none_val": return_none,
                "valid_val": return_value
            }
        )
    ...
    async def test_callable_exception_handling(self):
        ...
        tool = Tool(
            name="failing_tool",
            handler=handler,
            default_params={"value": failing_callable}
        )
...
class TestDefaultParamsWithAgents:
    ...
    async def test_agent_with_static_default_params(self):
        ...
        agent = Agent(
            name="static_agent",
            model="gpt-4o-mini",
            default_params={"context": "test_context"}
        )
    ...
    async def test_agent_default_params_with_system_prompt_templating(self):
        ...
        assert len(agent._system_prompt_callables) == 2  # os::getcwd and os::getpid
...
class TestDefaultParamsWithNestedTools:
    ...
    async def test_nested_tools_with_default_params(self):
        ...
        agent = Agent(
            name="nested_tools_agent",
            model="gpt-4o-mini",
            tools=[time_tool, counter_tool_instance]
        )
        ...
        prompt = Message.validate({
            "role": "user", 
            "content": "Please log the message 'System started' and then increment the counter"
        })
        result = agent(prompt=prompt)
        output = await result.collect()
    ...
    async def test_nested_agent_default_params(self):
        ...
        prompt = Message.validate({
            "role": "user",
            "content": "Please complete a simple task using the subagent"
        })
...
class TestDefaultParamsIntegration:
    ...
    async def test_default_params_error_handling_in_agent_context(self):
        ...
        agent = Agent(
            name="error_agent",
            model="gpt-4o-mini",
            tools=[tool_with_failing_default]
        )
