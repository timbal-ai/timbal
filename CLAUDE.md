...
- Connections are lazy and each lives in its own owner task (`close()` works from any task, any order; a dead transport is forgotten and the next call reconnects). The tool list is cached until `await server.close()`. Tools are sorted by name (stable LLM tools prefix → prompt-cache friendly)
